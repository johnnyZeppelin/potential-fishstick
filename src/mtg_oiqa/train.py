...
def evaluate(model: nn.Module, dataloader: DataLoader, device: torch.device) -> EvaluationResult:
    model.eval()
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            viewports = batch["viewports"].to(device)
            outputs = model(global_image, viewports)
...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            viewports = batch["viewports"].to(device)
            outputs = model(global_image, viewports)
            quality_loss = mse_loss(outputs["quality"], batch["mos"].to(device))
            distortion_loss = ce_loss(outputs["distortion_logits"], batch["distortion_level"].to(device))
            compression_loss = ce_loss(outputs["compression_logits"], batch["compression_type"].to(device))
            loss = (
